...
uniform float minViewElevation;
uniform float maxViewElevation;
...
void main(void)
{
	...
	if(elevation > maxElevation || elevation < minElevation) discard;
	...
	if(elevation >= 0)
	{
		float percent = (elevation - minViewElevation) / (maxViewElevation - minViewElevation);
		percent = clamp(percent, 0.0, 1.0);
		color = depthToChroma(1.0 - percent);
		alph *= 0.3;
	}
	else
	{
		float percent = (minElevation - elevation) / minElevation;
		percent = clamp(percent, 0.0, 1.0);
		color = ocean(percent);
	}
	
	gl_FragColor = vec4(light * color + specular * color, alph);
